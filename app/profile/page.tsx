...
export default function ProfilePage() {
  ...
  // 計算數據
  const totalScore = records.reduce((sum, record) => sum + record.score_earned, 0);
        ...
        <div className="bg-white rounded-3xl shadow-sm p-6 sm:p-10 mb-8 border border-slate-100">
          <div className="flex flex-col sm:flex-row items-center gap-6 mb-8">
            {user.user_metadata?.avatar_url ? (
              <img src={user.user_metadata.avatar_url} className="w-24 h-24 rounded-full border-4 border-emerald-50 shadow-sm" />
            ) : (
            ...
            <div className="text-center sm:text-left">
              ...
              <p className="text-slate-500 font-medium">台灣鳥類探索家</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 sm:gap-6">
            ...
            <div className="bg-sky-50 rounded-2xl p-6 text-center border border-sky-100/50">
              ...
              <div className="text-4xl sm:text-5xl font-black text-sky-700">
                {uniqueBirds} <span className="text-xl sm:text-2xl text-sky-400 font-bold">/ 703</span>
              </div>
            </div>
          </div>
        </div>
            ...
            return (
              <div key={record.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-100 hover:shadow-md transition-shadow">
                <div className="h-56 overflow-hidden bg-slate-100 relative">
                  <img src={record.photo_url} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
